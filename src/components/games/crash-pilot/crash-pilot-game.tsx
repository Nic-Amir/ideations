...
import { useCrashPilot } from '@/hooks/use-crash-pilot';
import {
  CRASH_HOUSE_EDGE,
  getMilestoneTable,
...
export function CrashPilotGame() {
      ...
      content: (
        <div className="space-y-2">
          ...
          <p className="text-xs text-on-subtle">
            Payouts carry a {(CRASH_HOUSE_EDGE * 100).toFixed(0)}% house margin at
    ...
    },
    {
      ...
      content: (
        <div className="space-y-2 text-sm text-on-subtle">
          <p>
            Your multiplier climbs on every tick the Deriv Crash index survives
            without crashing. The crash event is generated by Deriv&apos;s market
            engine — not by this game.
          </p>
          <p>
            A Crash N index crashes once every N ticks on average, and the odds
            are memoryless: every tick carries the same 1-in-N crash risk no
            matter how long the climb has lasted.
          </p>
          <p>
            Cash out any time to lock in the current multiplier. If the index
            crashes first, the stake is lost.
          </p>
        </div>
      ),
    },
  ...
  return (
    <GameShell infoSections={infoSections} showSymbolPicker={false}>
                ...
                <div className="pointer-events-none absolute inset-x-0 top-0 flex items-start justify-between px-2 pt-1.5">
                  ...
                  <div className="rounded bg-card/90 px-2 py-0.5 backdrop-blur-sm border border-border-subtle">
                    <span className="text-[10px] text-on-subtle">
                      Climb <span className="font-display tabular-nums text-on-prominent">{marketStreak}</span> ticks
                    </span>
                  </div>
                </div>
              </div>
        ...
        play={
          <div className="flex flex-col flex-1 min-h-0 px-4 py-3">
            ...
            <div className="relative flex-1 min-h-0 rounded-lg border border-border-subtle bg-subtle overflow-hidden">
              <CrashCurve curve={curve} phase={phase} className="absolute inset-0" />
      ...
      <ResultOverlay
        ...
        title={lastResult?.outcome === 'cashed_out' ? 'Cashed out' : 'Crashed'}
        subtitle={
          lastResult?.outcome === 'cashed_out'
            ? `Locked in at ${lastResult.multiplier.toFixed(2)}x.`
            : `The index crashed at ${lastResult?.multiplier.toFixed(2)}x.`
        }
        amount={
